...
1. Explain the difference between Relational Databases and SQL.
   ...
   SQL is a querying language made for accessing relational databases, which store the information to be queried via SQL.

2. Why do tables need a Primary Key?

   Without assigning primary keys to individual points of data, it would be hard/impossible to access a single record in particular. Primary keys ensure all data has a unique identifier that they can be queried by.

3. What is the name given to a table column that references the Primary Key on another table?

   A key in a table that points to a key in another table is called a foreign key. They are helpful in normalizing data, which reduces the likelihood of spelling errors, and allow us to link two tables together via querying to form a new one.

4. What do we need in order to have a _many to many_ relationship between two tables?

   We need an intermediary table to handle `many:many` relationships, where the intermediary table would contain foreign keys to the two (or more) tables to be linked.
